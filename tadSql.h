...
int mostraDados(TpBancoDeDados *pBanco) {
	int x = 3, y = 0, qtdeTab = 0, qtdeDados = 0;
	TpTabela *auxTab = pBanco->pTabelas;
	system("cls");
	gotoxy(x, y);
	printf("  Banco: %s", pBanco->nome);
	y+=2;
	while(auxTab) {
		x = 3;
...
int mostraSelecionado(TpBancoDeDados *pBanco, char comandosql[]) {
	int x = 3, y = 0, select[200], from[200], where[200], qtdeTab = 0, qtdeDados = 0, minI, maxI, valorC;
	float minF, maxF;
